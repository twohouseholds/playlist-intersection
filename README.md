...
2. Rename `.env.template` to `.env` and enter your Spotify Web API credentials:
...
3. Enter the Spotify playlist IDs of the playlists you want the intersection of in `data/config.yaml`. The playlist ID can be found in the playlist's link - e.g., if the link is `https://open.spotify.com/playlist/abc123`, the ID is `abc123`.
...
4. Install `uv` and run `uv sync --all-packages`.
