...
4. Enter the Spotify playlist IDs of the playlists you want the intersection of in `data/config.yaml`. The playlist ID can be found in the playlist's link - e.g., if the link is `https://open.spotify.com/playlist/abc123`, the ID is `abc123`.
...
6. Run the script: `uv run --env-file .env python scripts/playlist_intersection.py data/config.yaml`.
