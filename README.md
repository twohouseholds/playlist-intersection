...
3. Switch to the directory of your clone and run `uv sync --all-packages`.
4. Rename `.env.template` to `.env` and enter your Spotify Web API credentials:
...
5. In `data/`, rename `config.template.yaml` to `config.yaml` and configure the tool:
